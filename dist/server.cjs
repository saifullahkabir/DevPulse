...
var loginUserIntoDB = async (payload) => {
  ...
  const accessToken = import_jsonwebtoken.default.sign(jwtpayload, config_default.jwt_access_secret, {
    expiresIn: "7d"
  });
...
var getSingleIssue = async (req, res) => {
  try {
    ...
    return sendResponse_default(res, {
      statusCode: error instanceof Error && error.message === "Issue not found" ? import_http_status_codes2.StatusCodes.NOT_FOUND : import_http_status_codes2.StatusCodes.INTERNAL_SERVER_ERROR,
      success: false,
      message: error instanceof Error ? error.message : "Something went wrong",
      error
    });
...
var auth = (...roles) => {
  return async (req, res, next) => {
    try {
      ...
      if (userData.rows.length === 0) {
        return sendResponse_default(res, {
          statusCode: import_http_status_codes3.StatusCodes.NOT_FOUND,
          success: false,
      ...
      if (roles.length && !roles.includes(user.role)) {
        return sendResponse_default(res, {
          statusCode: import_http_status_codes3.StatusCodes.FORBIDDEN,
          success: false,
